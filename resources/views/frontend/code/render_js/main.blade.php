@extends('layouts.frontend.index')

@section('content')
    <h1 id="head1">Words Drill</h1>
    <div class="login-main clearfix col-md-12">
        <form id="frmPassword" novalidate="novalidate">
            <div class="pd10 clearfix border-bottom-solid">
                <div class="col-md-2 pd_t5 txt16">Form Id</div>
                <div class="col-md-2">
                    <input type="text" name="formId" id="formId" class="form-control">
                </div>

                <div class="col-md-4">
                    <div class="col-md-4 txt16">Confirm Id</div>
                    <div class="col-md-8 txt16">
                        <input type="text" name="confirmId" id="confirmId" class="form-control">
                    </div>
                </div>

                <div class="col-md-4">
                    <div class="col-md-4 txt16">Submit Id</div>
                    <div class="col-md-8 txt16">
                        <input type="text" name="confirmId" id="confirmId" class="form-control">
                    </div>
                </div>

            </div>

            <div class="pd10 border-bottom-solid clearfix">
                <div class="col-md-2 pd_t5 txt16">Field 1</div>
                <div class="col-md-3">
                    <input type="text" name="field1" id="field1" class="form-control">
                </div>
            </div>

            <div class="pd10 border-bottom-solid clearfix">
                <div class="col-md-2 pd_t5 txt16"></div>
                <div class="col-md-10">
                    <a href="javasript:void(0)" id="addLine" title="Add a line"><img src="/front/images/plus.png" class="cs-plus mg_r10" alt="Add a line">Add a line</a>
                    <a href="javascript:void(0)" id="removeLine"onclick="remove_group('group_work_experience')" class="c-minus" title="Remove a line" style="margin-left:50px">
                        <img src="/front/images/minus.png" class="cs-plus mg_r10" alt="Remove a line">Remove a line
                    </a>
                </div>
            </div>

            <div class="pd10  clearfix">
            </div>

            <div class="pd10  clearfix">
                <div class="col-md-2 pd_t5 txt16"></div>
                <div class="col-md-3">
                    <label for="create_script" class="custom-file-upload btn borange fl_l txt-white clr-both mg_r10">Create Script</label>
                    <input type="hidden" id="create_script" name="create_script" class="form-control">
                </div>
            </div>
        </form>
    </div>

    <script>
        $(document).ready(function () {
            $('#create_script').click(function(){
                console.log('click');
            });

            $('#plus_education').on("click", function (e) {
                e.preventDefault();
                row_education++;
                $.ajax({
                    type: 'post',
                    url: '{{route("code.insert.row.js")}}',
                    data: {'type':'education','row':row_education},
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    success: function (result) {
                        $('#education_list').append(result);
                        check_input();
                    }
                });
            });
        });
    </script>
    <style>
        input {
            max-width:150px;
        }
    </style>
@endsection